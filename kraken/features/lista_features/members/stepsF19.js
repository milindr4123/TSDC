...
When('I fill just label field', async function(){
    let labelInput = await this.driver.$('input[type="search"]');
    await labelInput.setValue('TestLabel');
}) 
...
Then('I check the label from label list', async function(){
    let lalbelSpan = await this.driver.$('span[data-test-selected-token]');
    let text = await lalbelSpan.getText();
    expect(text).to.equal('TestLabel');
})  
