...
When('I edit just name field', async function(){
    let memberName = await this.driver.$('input[data-test-input="member-name"]');
    await memberName.setValue('');
    return await memberName.setValue('Edit member test');
})  

When('I edit just email field', async function(){
    let memberEmail = await this.driver.$('input[data-test-input="member-email"]');
    await memberEmail.setValue('');
    return await memberEmail.setValue('editmembertest@prueba.com');
})  
...
Then('I check the edited name from member list', async function(){
    let listElements = await this.driver.$$('h3.gh-members-list-name');
    listElements.forEach(async (element) => {
        let text = await element.getText();
        if(text == 'Edit member test') expect(text).to.equal('Edit member test');
    })
})  

Then('I check the edited email from member list', async function(){
    let listElements = await this.driver.$$('p.gh-members-list-email');
    listElements.forEach(async (element) => {
        let text = await element.getText();
        if(text == 'editmembertest@prueba.com') expect(text).to.equal('editmembertest@prueba.com');
    })
})  

//Los siguientes pasos no van en el escenario pero se incluyen para mejor manejo de las pruebas

When('I navigate to the member by edited email', async function(){
    let listElementPost = await this.driver.$$('p.gh-members-list-email');
    for (const element of listElementPost) {
        let text = await element.getText();
        if (text == 'editmembertest@prueba.com') {
            return await element.click();
        }
    }
}) 
