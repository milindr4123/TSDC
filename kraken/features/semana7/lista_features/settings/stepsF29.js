...
When('I edit the title field', async function(){
    let element = await this.driver.$$('section[data-testid="add-recommendation-modal"] input[type="text"]')
    await element[0].setValue('');
    return await element[0].setValue('Facebook edit recommendation');
})
...
Then('I check the recommendations list', async function(){
    let element = await this.driver.$$('tr[data-testid="recommendation-list-item"]')
    expect(element.length > 0).to.equal(true);
})

//no van en el escenario

When('I click on list item', async function(){
