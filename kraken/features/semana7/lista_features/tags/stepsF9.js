...
When('I edit just tag name field', async function (){
    ...
    return await editTagElement.setValue('#EditTestAprioriTag');
})
...
Then('I check name from tag', async function (){
    let nameElement = await this.driver.$('a[href="#/tags/hash-testaprioritag/"] h3');
    expect(await nameElement.getText()).to.equal('#EditTestAprioriTag');
})
