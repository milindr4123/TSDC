...
async function recursiveExploration(page, link, depth, parentState){
  ...
  if(link.includes(baseUrl)){ //Only explore pages of the specified domain
    ...
    if(!!body) {
      ...
      if((link == "http://localhost:2368/ghost/#/signin" || link == "http://localhost:2368/ghost/")){
        await init(page);
      }
...
async function init(page){
  ...
  await page.keyboard.type("prueba@prueba.com");
  ...
  await recursiveExploration(page, page.url(), 0, -1);
}
